...
1. 在微信公众平台注册一个小程序，取得公开的 `AppID`。
2. 安装并登录微信开发者工具，选择“导入项目”，目录选择本文件夹。
3. 把 `project.config.json` 中的 `touristappid` 替换为你的小程序 `AppID`。
4. 在开发者工具中开通“云开发”，创建一个云环境。
5. 如未设置默认环境，把云环境 ID 填入 `miniprogram/config.js` 的 `cloudEnvId`。
6. 在云开发数据库中新建集合 `subscriptions`，安全规则设置为不允许客户端直接读写；业务数据只通过云函数访问。
7. 在开发者工具的“云函数”目录中，右键 `subscriptionService`，选择“上传并部署：云端安装依赖”。
8. 编译项目。首次打开时会自动完成微信身份识别，用户无需点击登录。
