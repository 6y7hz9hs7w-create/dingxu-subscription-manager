...
**Creative North Star: "会自己整理的数字手账"**

订序把记账网站的可信和年轻工具的鲜活放在同一张“数字账页”上。金额、续费日和状态永远先于装饰；电紫负责行动与导航，薄荷、黄色和珊瑚色像手账标签一样快速区分生命周期。

视觉保持清晰、平整且有节奏，不使用模糊玻璃、堆叠指标墙或与数据无关的装饰。桌面端像一张宽阔账页，移动端则自然折叠为单列操作流。
...
桌面端使用最大 1120px 的单一账本工作区，页头、摘要、导航和流水按纵向阅读顺序展开。摘要采用一个主金额加两个次指标的非等权布局。移动端在 760px 以下切为单列：主金额独占一行，次指标并排，操作按钮保持至少 42px 高。常用节奏为 8px 倍数，区块间距明显大于组件内部间距。
...
### Shadow Vocabulary
- **工作区环境影：** `0 28px 70px rgba(39, 32, 89, .14)`，仅用于桌面端主账页。
- **主操作浮层影：** `0 12px 26px rgba(93, 77, 245, .22)`，用于主按钮及其悬停反馈。
